...
func Setup(bot shared.Bot) {
	...
	// frankChain := gomarkov.NewChain(1)
	// patChain := gomarkov.NewChain(1)
	// audienceChain := gomarkov.NewChain(1)

	// // build the chain. Each source has newlines and spaces.
	// // newlines are end of line markers, spaces are word separators.
	// //
	// gerryChain.Add(strings.Split(GerryTextSource, " "))
	// frankChain.Add(strings.Split(FrankTextSource, " "))
	// patChain.Add(strings.Split(PatTextSource, " "))
	// audienceChain.Add(strings.Split(AudienceTextSource, " "))

	// //The chain is JSON serializable
	// gerryChainJson, _ := json.Marshal(gerryChain)
	// err := os.WriteFile("gerryModel.json", gerryChainJson, 0644)
	// if err != nil {
	// 	fmt.Println(err)
	// }

	// frankChainJson, _ := json.Marshal(frankChain)
	// err = os.WriteFile("frankModel.json", frankChainJson, 0644)
	// if err != nil {
	// 	fmt.Println(err)
	// }

	// patChainJson, _ := json.Marshal(patChain)
	// err = os.WriteFile("patModel.json", patChainJson, 0644)
	// if err != nil {
	// 	fmt.Println(err)
	// }

	// audienceChainJson, _ := json.Marshal(audienceChain)
	// err = os.WriteFile("audienceModel.json", audienceChainJson, 0644)
	// if err != nil {
	// 	fmt.Println(err)
	// }
...
func Call(bot shared.Bot, context shared.MessageContext, arguments []string, config shared.Config) error {
	...
	if arguments[0] == "train" {
		chain, err := buildModel(person)
		if err != nil {
			log.Println(err)
			return err
		}
		saveModel(chain, person)
	} else {
		chain, err := loadModel(person)
		if err != nil {
			log.Println(err)
			return err
		}
		tokens = generateSentence(chain)
	}

	finalString := strings.Join(tokens, " ")

	bot.Send(context, finalString)
	return nil
}

func buildModel(person string) (*gomarkov.Chain, error) {
	text := sources[person]
...
func loadModel(person string) (*gomarkov.Chain, error) {
	var chain gomarkov.Chain
	data, err := os.ReadFile(fmt.Sprintf("%sModel.json", person))
	if err != nil {
		return nil, err
	}
...
func saveModel(chain *gomarkov.Chain, person string) {
	jsonObj, _ := json.Marshal(chain)
	err := os.WriteFile(fmt.Sprintf("%sModel.json", person), jsonObj, 0644)
	if err != nil {
		log.Println(err)
	}
}
...
func generateSentence(chain *gomarkov.Chain) []string {
	tokens := []string{gomarkov.StartToken}
